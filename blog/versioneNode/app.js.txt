...
// attivazione middleware session
app.use(bodyParser.json()); // Parses the text as JSON and exposes the resulting object on req.body.
app.use(bodyParser.urlencoded({extended: true})); // Parses the text as URL encoded data (which is how browsers tend to send form data from regular forms set to POST) 
app.use(cookeParser());

app.use(session({ secret: 'crema', resave: false, saveUninitialized: true, }));

app.use(expressValidator()); // valid data type
